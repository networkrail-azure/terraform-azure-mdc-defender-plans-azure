locals {
  va_type = jsonencode({
    "vaType" = {
      "value" = var.vulnerabilityAssessmentProvider
    }
  })
...
data "azurerm_policy_definition" "vm_policies" {
  for_each = contains(var.mdc_plans_list, "VirtualMachines") && var.create_policy_assignments && var.enableAscForServers == "DeployIfNotExists" && var.enableAscForServersVulnerabilityAssessments == "DeployIfNotExists" ? local.virtual_machine_policies : {}
...
data "azurerm_role_definition" "vm_roles" {
  for_each = contains(var.mdc_plans_list, "VirtualMachines") && var.create_policy_assignments && var.enableAscForServers == "DeployIfNotExists" && var.enableAscForServersVulnerabilityAssessments == "DeployIfNotExists" ? local.virtual_machine_roles : {}
